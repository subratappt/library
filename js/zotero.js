function ZoteroManager(levels, meta) {
	...
	function createMetaTable(meta) {
		...
		var tbdy = document.createElement('div');
		...
		var pubJ = document.createElement('I');
		var yr;
		var vol = document.createElement('strong');
		var iss;
		var abs;
		var ath;
		var pag;
		...
		for (var i = 0; i < mlen; i++) {
			...
			if (key == "Volume") {
				vol.appendChild(document.createTextNode(val));
			}
			if (key == "Journal Abbreviation") {
				pubJ.appendChild(document.createTextNode(val + ", "));
				p = 1;
			}
			else if (key == "Publication Title" && p == 0) {
				pubJ.appendChild(document.createTextNode(val + ", "));
			}
			if (key == "Issue") {
				iss = val;
			...
			if (key == "Pages") {
				pag = document.createTextNode(", " + val);
			}
			if (key == "Date") {
				var dd = val;
				dat = document.createTextNode(" (" + dd + ")");
			}
			...
			if (key == 'DOI') {
				...
				anch.setAttribute('href', "https://doi.org/" + val + "\n");
				anch.appendChild(document.createTextNode(val));
				doi.appendChild(anch);
				l = 1;
			}
			if (key == 'URL' && l == 0) {
				var anch = document.createElement('a');
		...
		var athsec = document.createElement('div');
		athsec.classList.add("authorList");
		athsec.appendChild(document.createTextNode(ath))
		tbl.appendChild(athsec);
		var pubDetail = document.createElement('span');
		pubDetail.appendChild(pubJ);
		pubDetail.appendChild(vol)
		pubDetail.appendChild(pag);
		pubDetail.appendChild(dat);

		tbl.appendChild(pubDetail);
		tbl.appendChild(doi);
		var absSec = document.createElement('div');
		absSec.classList.add("abstract");
		absSec.appendChild(document.createTextNode(abs));
		tbl.appendChild(absSec);
	...
	function headerPane(node, root) {
		...
		btn.appendChild(document.createTextNode('View PDF'));
		topElem = document.createElement('div');
		topElem.appendChild(btn);
		topElem.classList.add('nav-item');
		topElem.classList.add('padTopBot');
		topPanel.appendChild(topElem);
	...
	function createMain(node) {
		...
		if (hasContent) {
			...
			noc.appendChild(document.createTextNode(node.text));
